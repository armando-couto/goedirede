...
func TestEEFI(t *testing.T) {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo032CabecalhoMatriz(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo035AjustesNetEDesagendados(linha, 0); retorno.TipoDeRegistro == 0 {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo036Antecipacoes(linha, 0); retorno.TipoDeRegistro == 0 {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo037TotalizadorDeCreditos(linha); retorno.TipoDeRegistro == 0 {
		t.Errorf("Erro no teste PopularRegistroTipo037TotalizadorDeCreditos(linha)")
	}

	linha = ""
	if retorno := leitor_edi_rede.PopularRegistroTipo038AjustesADebitoViaBanco(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo040Serasa(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo041AVS(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo042SecureCode(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo043AjustesACreditoExtratoEletronicoFinanceiro(linha, 0); retorno.TipoDeRegistro == 0 {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo044DebitosPendentes(linha); retorno.TipoDeRegistro == 0 {
		t.Errorf("Erro no teste PopularRegistroTipo044DebitosPendentes(linha)")
	}

	linha = ""
	if retorno := leitor_edi_rede.PopularRegistroTipo045DebitosLiquidados(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo049DesagendamentosDeParcelas(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo052TrailerDoArquivo(linha); retorno.TipoDeRegistro == 0 {
	...
	linha = ""
	if retorno := leitor_edi_rede.PopularRegistroTipo054AjustesADebitoViaBancoECommerce(linha); retorno.TipoDeRegistro == 0 {
		t.Errorf("Erro no teste PopularRegistroTipo054AjustesADebitoViaBancoECommerce(linha)")
	}

	linha = ""
	if retorno := leitor_edi_rede.PopularRegistroTipo055DebitosPendentes(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := leitor_edi_rede.PopularRegistroTipo056DebitosLiquidadosECommerce(linha); retorno.TipoDeRegistro == 0 {
		t.Errorf("Erro no teste PopularRegistroTipo056DebitosLiquidadosECommerce(linha)")
	}

	linha = ""
	if retorno := leitor_edi_rede.PopularRegistroTipo057DesagendamentosDeParcelasECommerce(linha); retorno.TipoDeRegistro == 0 {
