...
import (
	"github.com/armando-couto/goedirede"
	"testing"
)
...
func TestEEFI(t *testing.T) {
	...
	if retorno := goedirede.PopularRegistroTipo030CabecalhoDoArquivo(linha); retorno.TipoDeRegistro == 0 {
		t.Errorf("Erro no teste PopularRegistroTipo030CabecalhoDoArquivo(linha)")
	}

	linha = "032080956998TESTE                 "
	if retorno := goedirede.PopularRegistroTipo032CabecalhoMatriz(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo034Creditos(linha, 0); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo035AjustesNetEDesagendados(linha, 0); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo036Antecipacoes(linha, 0); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo037TotalizadorDeCreditos(linha); retorno.TipoDeRegistro == 0 {
		t.Errorf("Erro no teste PopularRegistroTipo037TotalizadorDeCreditos(linha)")
	}

	linha = ""
	if retorno := goedirede.PopularRegistroTipo038AjustesADebitoViaBanco(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo040Serasa(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo041AVS(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo042SecureCode(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo043AjustesACreditoExtratoEletronicoFinanceiro(linha, 0); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo044DebitosPendentes(linha); retorno.TipoDeRegistro == 0 {
		t.Errorf("Erro no teste PopularRegistroTipo044DebitosPendentes(linha)")
	}

	linha = ""
	if retorno := goedirede.PopularRegistroTipo045DebitosLiquidados(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo049DesagendamentosDeParcelas(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo050TotalizadorMatriz(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo052TrailerDoArquivo(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo053AjustesNetEDesagendamentosECommerce(linha); retorno.TipoDeRegistro == 0 {
		t.Errorf("Erro no teste RegistroTipo053AjustesNetEDesagendamentosECommerce(linha)")
	}

	linha = ""
	if retorno := goedirede.PopularRegistroTipo054AjustesADebitoViaBancoECommerce(linha); retorno.TipoDeRegistro == 0 {
		t.Errorf("Erro no teste PopularRegistroTipo054AjustesADebitoViaBancoECommerce(linha)")
	}

	linha = ""
	if retorno := goedirede.PopularRegistroTipo055DebitosPendentes(linha); retorno.TipoDeRegistro == 0 {
	...
	if retorno := goedirede.PopularRegistroTipo056DebitosLiquidadosECommerce(linha); retorno.TipoDeRegistro == 0 {
		t.Errorf("Erro no teste PopularRegistroTipo056DebitosLiquidadosECommerce(linha)")
	}

	linha = ""
	if retorno := goedirede.PopularRegistroTipo057DesagendamentosDeParcelasECommerce(linha); retorno.TipoDeRegistro == 0 {
